async function scrapeComments(maxComments) {
    ...
    maxComments = 100;
    try {
        console.log("初始HTML状态:", document.body.innerHTML.substring(0, 500) + "...");
        console.log("尝试获取评论总数");
        let commentCount = maxComments;
        const selectors = ["span[class*=\"tagItem\"][class*=\"isSelected\"]", "span:contains(\"全部(\")", ".rate-counter", ".comment-count", ".review-count"];
        function parseCommentCount1(text) {
            ...
            if (commentCount1) {
                ...
                if (commentCount11.includes("万")) {
                    console.log(`检测到"万"字，评论数量肯定超过100，使用最大值 ${maxComments}`);
                    return maxComments;
                }
                ...
                if (commentCount11.includes("+")) {
                    console.log(`检测到"+"号，评论数量可能很大，使用最大值 ${maxComments}`);
                    return maxComments;
                }
            }
                ...
                if (selector.includes(":contains(")) {
                    ...
                    for (const spanElement of spanElements) {
                        if (spanElement.textContent.includes("全部(")) {
                            console.log(`找到可能包含评论数的元素: "${spanElement.textContent}"`);
        ...
        const infoSelectors = ["p.tabDetailItemTitle--RW7pKjfx", "p[class*=\"tabDetailItemTitle\"]", "p[data-spm-anchor-id*=\"pc_detail.29232929.202205\"]", "p:contains(\"参数信息\")"];
        ...
        const parseCommentCount = ["div[class*=\"ShowButton\"]", "a:contains(\"全部评价\")", "button:contains(\"全部评价\")", "div:contains(\"全部评价\")", "span:contains(\"全部评价\")"];
        ...
        if (parseCommentCount2) {
            ...
            if (leftDrawer) {
                ...
                const commentsContainer = leftDrawer.querySelector("div[class*=\"comments--\"][class*=\"beautify-scroll-bar\"], div[class*=\"comments--\"], .beautify-scroll-bar, div[class*=\"beautify-scroll\"]");
                if (commentsContainer) {
                    ...
                    if (comments && comments.length > 0) {
                        console.log(`成功从评论容器提取 ${comments.length} 条评论 (总数: ${commentCount})`);
                        closeCommentOverlay();
                        chrome.runtime.sendMessage({
                            action: "updateStatus",
                        ...
                        if (commentsFromDrawer && commentsFromDrawer.length > 0) {
                            console.log(`成功从leftDrawer提取 ${commentsFromDrawer.length} 条评论 (总数: ${commentCount})`);
                            closeCommentOverlay();
                            chrome.runtime.sendMessage({
                                action: "updateStatus",
                    ...
                    console.log("在leftDrawer中未找到评论容器，尝试从整个leftDrawer中提取评论");
                    let commentsFromDrawer = extractComments(leftDrawer);
                    if (commentsFromDrawer && commentsFromDrawer.length > minComments) {
                        console.log(`评论数量超过期望值，截取前 ${minComments} 条`);
                        commentsFromDrawer = commentsFromDrawer.slice(0, minComments);
            ...
            const commentSelectors = ["div[class*=\"Comments--\"]", "div[class*=\"comments--\"]", "div.content--ew3Y4lVg", ".tb-revbd", ".rate-grid", ".comment-list", ".beautify-scroll-bar", ".modal-content", ".dialog-content", ".popup-content", ".overlay-content"];
            ...
            if (commentsPanel) {
                ...
                if (comments && comments.length > 0) {
                    console.log(`成功从评论面板提取 ${comments.length} 条评论 (总数: ${commentCount})`);
                    chrome.runtime.sendMessage({
                        action: "updateStatus",
                        status: `成功抓取 ${comments.length}/${commentCount} 条评论`
                    });
        ...
        let commentsPanel = document.querySelector("div[class*=\"Comments--\"], div.content--ew3Y4lVg, .tb-revbd, .rate-grid, .comment-list");
        if (commentsPanel) {
        ...
        if (parseCommentCount3 && parseCommentCount3.length > 0) {
            console.log(`成功直接从页面提取 ${parseCommentCount3.length} 条评论 (总数: ${commentCount})`);
            chrome.runtime.sendMessage({
                action: "updateStatus",
        ...
        if (parseCommentCount4 && parseCommentCount4.length > 0) {
            console.log(`成功使用备用方法提取 ${parseCommentCount4.length} 条评论 (总数: ${commentCount})`);
            chrome.runtime.sendMessage({
                action: "updateStatus",
...
function closeCommentOverlay() {
    try {
        ...
        const closeSelectors = [".closeIcon--NdjsxNY9", "img[src*=\"closeIcon\"]", "img[src*=\"close\"]", "img[class*=\"closeIcon\"]", "div[class*=\"closeIcon\"]", "span[class*=\"closeIcon\"]", "button[class*=\"closeIcon\"]", "button[class*=\"close\"]", "div[class*=\"close\"]", "span[class*=\"close\"]"];
        ...
        if (closeButton) {
            ...
            console.log("未找到关闭按钮");
            const leftDrawer = document.querySelector(".leftDrawer--4H_p9fnt, div[class*=\"leftDrawer--\"]");
            if (leftDrawer) {
                for (const selector of closeSelectors) {
                    try {
...
function loadAllComments(commentsPanel, maxComments = 100) {
    return new Promise(async resolve => {
        ...
        const maxScrollAttempts = maxComments > 1000 ? 1000 : maxComments > 100 ? 500 : maxComments < 10 ? 50 : 200;
        console.log(`设置最大滚动尝试次数: ${maxScrollAttempts}`);
        let commentCount = 0;
        let scrollFailures = 0;
        const maxFails = 15;
        function getComments() {
            const selectors = [".Comment--KkPcz74T", "div[class*=\"Comment--\"]", ".comment-item", "div.contentWrapper--uAdAlCgC", ".rate-item", ".tb-revbd .tb-r-review", ".review-item", ".comment-container", ".comment-list > div", ".comment-list > li", "div[class*=\"rate-item\"]", "div[class*=\"review-item\"]", "div[class*=\"comment-item\"]", "div[data-index]"];
            let commentElements = [];
            for (const selector of selectors) {
                try {
        ...
        function scrollHandler(attempt = 0) {
            ...
            console.log(`滚动尝试 #${attempt}: 当前评论数量 ${commentCount1}/${maxComments}`);
            ...
            if (commentCount1 >= maxComments || attempt >= maxScrollAttempts || scrollFailures >= maxFails && attempt > 50) {
                if (commentCount1 >= maxComments) {
                    console.log(`已达到目标评论数量: ${commentCount1}/${maxComments}`);
                } else {
                    if (attempt >= maxScrollAttempts) {
                        console.log(`已达到最大滚动尝试次数: ${attempt}/${maxScrollAttempts}`);
                    } else {
...
function extractComments(commentsPanel, bestSelector) {
    console.log("开始提取评论");
    const commentSelectors = [".Comment--KkPcz74T", "div[class*=\"Comment--\"]", ".comment-item", "div.contentWrapper--uAdAlCgC", ".rate-item", ".tb-revbd .tb-r-review", ".review-item", ".comment-container", ".comment-list > div", ".comment-list > li", "div[class*=\"rate-item\"]", "div[class*=\"review-item\"]", "div[class*=\"comment-item\"]", "div[data-index]"];
    let comments = [];
    bestSelector = "";
    if (commentsPanel) {
        for (const selector of commentSelectors) {
            try {
                ...
                if (elements && elements.length > 0) {
                    if (elements.length > comments.length) {
                        ...
                        console.log(`在评论面板中使用选择器 ${selector} 找到 ${elements.length} 条评论 (当前最佳)`);
                    } else {
                        console.log(`在评论面板中使用选择器 ${selector} 找到 ${elements.length} 条评论`);
                    }
                }
            } catch (e) {
                console.error(`选择器 ${selector} 出错:`, e);
        ...
        console.log(`最终使用选择器 ${bestSelector} 找到 ${comments.length} 条评论`);
        if (comments.length === 0) {
            console.log("在评论面板中未找到评论元素，尝试查找所有子元素");
            const childElements = commentsPanel.children;
            if (childElements && childElements.length > 0) {
                comments = Array.from(childElements).filter(child => !child.className.includes("footer") && !child.className.includes("loading") && !child.className.includes("title"));
                console.log(`在评论面板中找到 ${comments.length} 个可能的评论元素(直接子元素)`);
            }
        }
        if (comments.length === 0) {
            ...
            try {
                ...
                if (elements && elements.length > 0) {
                    if (elements.length > comments.length) {
                        comments = Array.from(elements);
                        bestSelector = selector;
                        console.log(`在文档中使用选择器 ${selector} 找到 ${elements.length} 条评论 (当前最佳)`);
                    } else {
                        console.log(`在文档中使用选择器 ${selector} 找到 ${elements.length} 条评论`);
                    }
                }
            } catch (e) {
                console.error(`选择器 ${selector} 出错:`, e);
    ...
    if (comments.length === 0) {
        ...
        const allDivs = commentsPanel ? commentsPanel.querySelectorAll("div") : document.querySelectorAll("div");
        comments = Array.from(allDivs).filter(div => {
            const divText = div.textContent;
            const hasPattern = divText.includes("t**") || divText.match(new RegExp("[a-z]\\*\\*\\d*", "i"));
            const dateMatch = divText.match(new RegExp("\\d{4}年\\d{1,2}月\\d{1,2}日", ""));
    ...
    comments.forEach((element, index) => {
        try {
            ...
            console.log(`评论 ${index + 1} 完整文本:`, commentText.substring(0, 100) + (commentText.length > 100 ? "..." : ""));
            let userName = "";
            const userSelectors = ["div[class*=\"userName\"] span", ".userName--mmxkxkd0 span", "div[class*=\"userName--\"] span", ".user-name span", ".username span", ".name span", "div[class*=\"userName\"]", ".userName--mmxkxkd0", "div[class*=\"userName--\"]", ".user-name", ".username", ".name"];
            ...
            let purchaseDate = "";
            let style = "";
            const metaSelectors = ["div[class*=\"meta\"]", ".meta--TDfRej2n", "div[class*=\"meta--\"]", ".date", ".spec", "div[class*=\"date\"]", "div[class*=\"spec\"]", "span[class*=\"date\"]", "span[class*=\"spec\"]"];
                ...
                try {
                    ...
                    if (metaElement && metaElement.textContent.trim()) {
                        ...
                        if (metaParts.length >= 1) {
                            const dateMatch = metaParts[0].match(new RegExp("\\d{4}年\\d{1,2}月\\d{1,2}日", ""));
                            if (dateMatch) {
            ...
            if (!purchaseDate) {
                const dateMatch = commentText.match(new RegExp("\\d{4}年\\d{1,2}月\\d{1,2}日", ""));
                if (dateMatch) {
                    purchaseDate = dateMatch[0];
            ...
            if (!style) {
                const extractedData = [commentText.match(new RegExp("规格[：:]\\s*([^，。,]+)", "")), commentText.match(new RegExp("颜色分类[：:]\\s*([^，。,]+)", "")), commentText.match(new RegExp("型号[：:]\\s*([^，。,]+)", "")), purchaseDate && commentText.includes(purchaseDate) && commentText.substring(commentText.indexOf(purchaseDate) + purchaseDate.length).match(new RegExp("[^，。,:：]+", "")), commentText.match(new RegExp("左右排烟\\s*\\/\\s*天然气\\s*\\/\\s*([^，。,]+)", "")), commentText.match(new RegExp("([【\\[].+[】\\]])", "")), commentText.match(new RegExp("(\\d+m³\\/min\\s*[黑白金银铜灰色]+)", ""))];
                for (const styleEntry of extractedData) {
                    if (styleEntry && (styleEntry[1] || styleEntry[0]) && (styleEntry[1] || styleEntry[0]).trim()) {
            ...
            let commentContent = "";
            const commentSelectors1 = [".content", ".text", ".comment", "div[class*=\"content\"]", "span[class*=\"content\"]", "div[class*=\"text\"]", "span[class*=\"text\"]", "div[class*=\"comment\"]", "span[class*=\"comment\"]"];
            for (const selector of commentSelectors1) {
                try {
                    ...
                    if (selectedElement && selectedElement.textContent.trim()) {
                        ...
                        console.log(`从选择器 ${selector} 找到内容: ${commentContent.substring(0, 50)}...`);
                        break;
                    }
                } catch (e) {
            ...
            if (!commentContent) {
                ...
                commentContent = commentContent.replace(new RegExp("有用\\s*\\(\\d+\\)", "g"), "").replace(new RegExp("回复\\s*\\(\\d+\\)", "g"), "").replace(new RegExp("举报", "g"), "").replace(new RegExp("此用户没有填写评价", "g"), "").replace(new RegExp("追加评论", "g"), "").replace(new RegExp("评价方式：", "g"), "").replace(new RegExp("评价时间：", "g"), "").replace(new RegExp("购买时间：", "g"), "").replace(new RegExp("\\s*·\\s*", "g"), "");
                commentContent = commentContent.trim();
                console.log(`从完整文本提取内容: ${commentContent.substring(0, 50)}...`);
            }
            if (userName && commentContent && commentContent !== "此用户没有填写评价") {
                ...
                console.log(`成功提取第 ${index + 1} 条评论:`, userName, purchaseDate, style);
            } else {
                console.log(`跳过无效评论:`, userName, commentContent.substring(0, 30) + "...");
            }
        } catch (e) {
...
function scrapeCommentsFromCurrentPage() {
    ...
    try {
        ...
        if (leftDrawer) {
            console.log("找到leftDrawer容器");
            const commentsContainer = leftDrawer.querySelector(".comments--vOMSBfi2.beautify-scroll-bar, div[class*=\"comments--\"][class*=\"beautify-scroll-bar\"], .comments--vOMSBfi2, div[class*=\"comments--\"], .beautify-scroll-bar, div[class*=\"beautify-scroll\"]");
            if (commentsContainer) {
                console.log("找到评论容器:", commentsContainer.className);
                const comments1 = commentsContainer.querySelectorAll(".Comment--KkPcz74T, div[class*=\"Comment--\"]");
        ...
        const maskSelectors = [".content--ew3Y4lVg", "div[class*=\"content--\"]", ".modal-content", ".dialog-content", ".popup-content", ".overlay-content", ".beautify-scroll-bar", "div[class*=\"beautify-scroll\"]"];
        ...
        const commentSelectors = ["div.用户评价", "div:has(> h2:contains(\"用户评价\"))", "div[class*=\"rate\"]", "div.tb-reviews", "div[class*=\"comment\"]", "div[class*=\"Comment\"]", "div[class*=\"review\"]", "div[class*=\"Review\"]", "div[class*=\"评价\"]", "div[class*=\"评论\"]"];
        ...
        const possibleComments = Array.from(allDivs).filter(div => {
            ...
            const containsPattern = textContent.includes("t**") || textContent.match(new RegExp("[a-z]\\*\\*\\d*", "i"));
            const dateMatch = textContent.match(new RegExp("\\d{4}年\\d{1,2}月\\d{1,2}日", ""));
        ...
        possibleComments.forEach((element, index) => {
            try {
                ...
                console.log(`评论 ${index + 1} 完整文本:`, commentText.substring(0, 100) + (commentText.length > 100 ? "..." : ""));
                let userName = "";
                const userSelectors = ["div[class*=\"userName\"] span", ".userName--mmxkxkd0 span", "div[class*=\"userName--\"] span", ".user-name span", ".username span", ".name span"];
                for (const selector of userSelectors) {
                    try {
                        const userElement = element.querySelector(selector);
                        if (userElement && userElement.textContent.trim()) {
                ...
                if (!userName) {
                    const username = commentText.match(new RegExp("[a-z]\\*\\*\\d+", "i"));
                    if (username) {
                        userName = username[0];
                        console.log(`从文本中提取用户名: ${userName}`);
                    }
                ...
                let purchaseDate = "";
                let style = "";
                const metaSelectors = ["div[class*=\"meta\"]", ".meta--TDfRej2n", "div[class*=\"meta--\"]", ".date", ".spec"];
                for (const selector of metaSelectors) {
                    try {
                ...
                if (!style) {
                    const extractedDetails = [commentText.match(new RegExp("规格[：:]\\s*([^，。,]+)", "")), commentText.match(new RegExp("颜色分类[：:]\\s*([^，。,]+)", "")), commentText.match(new RegExp("型号[：:]\\s*([^，。,]+)", "")), purchaseDate && commentText.includes(purchaseDate) && commentText.substring(commentText.indexOf(purchaseDate) + purchaseDate.length).match(new RegExp("[^，。,:：]+", "")), commentText.match(new RegExp("左右排烟\\s*\\/\\s*天然气\\s*\\/\\s*([^，。,]+)", "")), commentText.match(new RegExp("([【\\[].+[】\\]])", "")), commentText.match(new RegExp("(\\d+m³\\/min\\s*[黑白金银铜灰色]+)", ""))];
                    for (const detail of extractedDetails) {
                        if (detail && (detail[1] || detail[0]) && (detail[1] || detail[0]).trim()) {
                ...
                let content = "";
                const contentSelectors = [".content", ".text", ".comment", "div[class*=\"content\"]", "span[class*=\"content\"]", "div[class*=\"text\"]", "span[class*=\"text\"]", "div[class*=\"comment\"]", "span[class*=\"comment\"]"];
                for (const selector of contentSelectors) {
                    try {
                        const selectedElement = element.querySelector(selector);
                        if (selectedElement && selectedElement.textContent.trim()) {
                            content = selectedElement.textContent.trim();
                            console.log(`从选择器 ${selector} 找到内容: ${content.substring(0, 50)}...`);
                            break;
                        }
                    } catch (e) {
                        console.error(`内容选择器 ${selector} 出错:`, e);
                ...
                if (!content) {
                    ...
                    content = content.replace(new RegExp("有用\\s*\\(\\d+\\)", "g"), "").replace(new RegExp("回复\\s*\\(\\d+\\)", "g"), "").replace(new RegExp("举报", "g"), "").replace(new RegExp("此用户没有填写评价", "g"), "").replace(new RegExp("追加评论", "g"), "").replace(new RegExp("评价方式：", "g"), "").replace(new RegExp("评价时间：", "g"), "").replace(new RegExp("购买时间：", "g"), "").replace(new RegExp("\\s*·\\s*", "g"), "");
                ...
                if (userName && content && content !== "此用户没有填写评价") {
                    ...
                    console.log(`跳过无效评论:`, userName, content.substring(0, 30) + "...");
                }
            } catch (e) {
                console.error(`提取第 ${index + 1} 条评论时出错:`, e);
            }
...
function debugCommentElements() {
    ...
    console.log("--- 可能的评论按钮 ---");
    const commentButtons = [document.querySelector(".ShowButton--o4XEG7ih"), document.querySelector("div[class*=\"ShowButton\"]"), Array.from(document.querySelectorAll("div")).find(el => el.textContent.includes("全部评价")), document.querySelector(".tabDetailItemTitle--_KkXcMDu")];
    ...
    console.log("--- 可能的评论面板 ---");
    const commentPanels = [document.querySelector(".Comments--vOMSBfi2"), document.querySelector(".comments--vOMSBfi2"), document.querySelector(".beautify-scroll-bar"), document.querySelector("div[class*=\"Comments--\"]"), document.querySelector("div.content--ew3Y4lVg")];
    commentPanels.forEach((panel, index) => {
        if (panel) {
            console.log(`面板 ${index + 1} 存在:`, panel.className);
            const commentCount = panel.querySelectorAll(".Comment--KkPcz74T, div[class*=\"Comment--\"], .comment-item, div.contentWrapper--uAdAlCgC").length;
    ...
    const commentSelectors = [".Comment--KkPcz74T", "div[class*=\"Comment--\"]", ".comment-item", "div.contentWrapper--uAdAlCgC"];
    commentSelectors.forEach((selector, index) => {
        try {
            const comments = document.querySelectorAll(selector);
            console.log(`选择器 ${selector} 找到 ${comments.length} 条评论`);
            if (comments.length > 0) {
                console.log("第一条评论结构:", comments[0].outerHTML.substring(0, 200) + "...");
            }
        } catch (e) {
            console.error(`选择器 ${selector} 出错:`, e);
        }
    ...
    return new Promise(resolve => {
        try {
            ...
            if (leftDrawer) {
                ...
                if (commentsContainer) {
                    console.log("找到评论容器:", commentsContainer.className);
                    const commentsList = commentsContainer.querySelectorAll(".Comment--KkPcz74T, div[class*=\"Comment--\"]");
            ...
            console.log("尝试查找其他可能的遮罩容器");
            const selectors = [".content--ew3Y4lVg", "div[class*=\"content--\"]", ".modal-content", ".dialog-content", ".popup-content", ".overlay-content", ".beautify-scroll-bar", "div[class*=\"beautify-scroll\"]"];
            let overlay = null;
            for (const selector of selectors) {
                try {
                    const container = document.querySelector(selector);
                ...
                try {
                    if (selector.includes(":has(")) {
                        ...
                        for (const reviewContainer of allDivs) {
                            ...
                            if (reviewHeader && reviewHeader.textContent.includes("用户评价")) {
                                console.log("找到包含\"用户评价\"标题的容器");
                                const comments = extractComments(reviewContainer);
                                if (comments && comments.length > 0) {
                                    console.log(`从包含"用户评价"标题的容器中提取到 ${comments.length} 条评论`);
                        ...
                        if (commentContainers && commentContainers.length > 0) {
                            console.log(`找到 ${commentContainers.length} 个评论容器: ${selector}`);
                            for (const container of commentContainers) {
                                const comments = extractComments(container);
                                if (comments && comments.length > 0) {
                                    console.log(`从容器 ${selector} 中提取到 ${comments.length} 条评论`);
                                    comments1 = comments1.concat(comments);
                                }
                            }
                        }
            ...
            if (comments1.length > 0) {
                ...
                for (const comment of comments1) {
                    const commentId = `${comment.userName}-${comment.content.substring(0, 50)}`;
                    if (!commentSet.has(commentId)) {
                        commentSet.add(commentId);
                        uniqueComments.push(comment);
                    }
                }
                console.log(`从所有容器中共提取到 ${comments1.length} 条评论，去重后剩余 ${uniqueComments.length} 条`);
            ...
            const filterDivs = Array.from(allDivs).filter(div => {
                ...
                const hasSpecialContent = content.includes("t**") || content.match(new RegExp("[a-z]\\*\\*\\d*", "i"));
                const dateMatch = content.match(new RegExp("\\d{4}年\\d{1,2}月\\d{1,2}日", ""));
            ...
            filterDivs.forEach((element, index) => {
                try {
                    console.log(`处理第 ${index + 1}/${filterDivs.length} 个可能的评论元素`);
                    const commentText = element.textContent.trim();
                    ...
                    if (!style) {
                        const extractedPatterns = [commentText.match(new RegExp("规格[：:]\\s*([^，。,]+)", "")), commentText.match(new RegExp("颜色分类[：:]\\s*([^，。,]+)", "")), commentText.match(new RegExp("型号[：:]\\s*([^，。,]+)", "")), purchaseDate && commentText.includes(purchaseDate) && commentText.substring(commentText.indexOf(purchaseDate) + purchaseDate.length).match(new RegExp("[^，。,:：]+", "")), commentText.match(new RegExp("左右排烟\\s*\\/\\s*天然气\\s*\\/\\s*([^，。,]+)", "")), commentText.match(new RegExp("([【\\[].+[】\\]])", "")), commentText.match(new RegExp("(\\d+m³\\/min\\s*[黑白金银铜灰色]+)", ""))];
                        for (const pattern of extractedPatterns) {
                            if (pattern && (pattern[1] || pattern[0]) && (pattern[1] || pattern[0]).trim()) {
