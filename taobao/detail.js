async function scrapeTaobaoDetailData(price, originalPrice, salesCount, mainImage, coupons, couponsList, scrapingStatus, scrapingOptions, scrollToTop, getScrollPosition) {
    console.log("准备抓取淘宝商品详情数据");
    const isAtTop = window.scrollY < 50;
    const isTmallPage = window.location.href.includes("detail.tmall.com") || window.location.href.includes("tmall.com/item");
    if (!isAtTop) {
    ...
    let skuData = [];
    try {
        const getSkuOptions = typeof scrapeSkuOptions === "function" ? scrapeSkuOptions : window.skuScraper && window.skuScraper.scrapeSkuOptions;
        if (getSkuOptions) {
    ...
    const titleEl = document.querySelector(".mainTitle--ocKo1xwj, [class*=\"mainTitle--\"], .tb-detail-hd h1, .tb-main-title, .ItemHeader--title");
    const productTitle = titleEl ? titleEl.innerText.trim() : "";
    price = "";
    originalPrice = "";
    const priceSelectors = [".J_price", ".tm-price", ".J_priceStd", "#J_StrPrice", ".tb-rmb-num", ".mod-info-price .price", ".tb-detail-price .tb-rmb-num", "[class*=\"Price--small--\"]", "[class*=\"Price--large--\"]", "[class*=\"Price--normal--\"]", "[class*=\"Price--priceText--\"]", "[class*=\"Price--priceInteger--\"]", "[class*=\"Price--priceDecimal--\"]", "[class*=\"highlightPrice--\"] [class*=\"text--\"]"];
    ...
    const originalPriceSelectors = [".J_priceOri", ".J_originalPrice", ".mod-info-price .price-original", ".tb-detail-price .tb-rmb-num.tb-rmb-del", ".tm-price-panel .tm-price-panel-price-original", "[class*=\"Price--delete--\"]", "[class*=\"Price--deleted--\"]", "[class*=\"Price--original--\"]", "[class*=\"subPrice--\"] [class*=\"text--\"]:last-child"];
    ...
    if (salesDesc) {
        ...
        const salesMatch = salesInfo.match(new RegExp("已售\\s*([\\d万千百十亿]+\\+?)", ""));
    ...
    if (!salesCount) {
        const salesDesc1 = document.querySelector("[class*=\"summaryWrap--\"] [class*=\"salesDesc--\"]");
        if (salesDesc1) {
            const salesDesc11 = salesDesc1.innerText.trim();
            const salesMatch = salesDesc11.match(new RegExp("已售\\s*([\\d万千百十亿]+\\+?)", ""));
            if (salesMatch && salesMatch[1]) {
                salesCount = salesMatch[1];
            } else {
    ...
    const imageUrls = [];
    const thumbnailImages = document.querySelectorAll("[class*=\"thumbnailPic--\"], .tb-thumb li img, .tm-layout-list-item img");
    thumbnailImages.forEach(imgEl => {
        if (imgEl && imgEl.src) {
            ...
            imgSrc = imgSrc.replace(new RegExp("\\.jpg_\\d+x\\d+q\\d+\\.jpg", "g"), ".jpg");
            imgSrc = imgSrc.replace(new RegExp("\\.jpg_\\d+x\\d+\\.jpg", "g"), ".jpg");
    ...
    mainImage = "";
    const mainImageSelectors = ["[class*=\"mainPic--\"]", ".tb-booth img", ".tb-main-pic img", ".magnifier-preview img", ".preview-img img"];
    ...
    coupons = "";
    const mainImg = ["video source", ".lib-video video source", ".tb-video-player video source", ".mui-player video source", "[data-role=\"videoPlayer\"] video source"];
    ...
    const skuOptions = [];
    const scrapeCoupons = [];
    const fetchSKUOptions = ["[class*=\"tabDetailItem--\"] [class*=\"infoItem--\"]", ".tb-attributes .attributes-list li", ".tm-fcs-panel .tm-fcs-properties .tm-fcs-property", ".attributes-list .property-item"];
    for (const selector of fetchSKUOptions) {
        const elements = document.querySelectorAll(selector);
        if (elements && elements.length > 0) {
            elements.forEach(item => {
                const selectors = ["[class*=\"infoItemTitle--\"]", ".property-name", ".tb-property-type", ".tm-fcs-prop-key"];
                const contentSelectors = ["[class*=\"infoItemContent--\"]", ".property-value", ".tb-property-cont", ".tm-fcs-prop-value"];
                let titleEl = null;
    ...
    const SKUOptions = ["[class*=\"imageTextInfo--\"]", ".descV8-container", ".desc-root", "#J_DivItemDesc", "#description", ".tb-desc", ".detail-content", ".detail-desc", ".detail-desc-decorate"];
    for (const selector of SKUOptions) {
        const element = document.querySelector(selector);
        if (element) {
            const textList = Array.from(element.querySelectorAll("p, h1, h2, h3, h4, h5, h6, span, div")).filter(el => el.innerText && !el.querySelector("a") && el.className !== "descV8-hotArea").map(el => el.innerText.trim()).filter(text => text.length > 0);
            const shortTexts = [];
            ...
            textList.forEach(text => {
                ...
                for (const uniqueText of uniqueTexts) {
                    if (uniqueText === text || uniqueText.includes(text) || text.includes(uniqueText)) {
                        isDuplicate = true;
                        break;
                    }
                }
            ...
            const imageElements = element.querySelectorAll(".descV8-singleImage img, .descV8-singleImage-image");
            ...
            if (getScrollPosition && getScrollPosition.length > 500 || skuOptionsFunction.length > 10) {
                break;
            }
        }
    }
...
async function scrollTaobaoDetailPage() {
    ...
    return new Promise(async resolve => {
        try {
            ...
            console.log(`滚动到页面的25%位置: ${documentHeight * 0.25}/${documentHeight}`);
            await new Promise(r => setTimeout(r, 1500));
            window.scrollTo({
                top: documentHeight * 0.5,
                behavior: "smooth"
            });
            console.log(`滚动到页面的50%位置: ${documentHeight * 0.5}/${documentHeight}`);
            await new Promise(r => setTimeout(r, 1500));
            window.scrollTo({
                top: documentHeight * 0.75,
                behavior: "smooth"
            });
            console.log(`滚动到页面的75%位置: ${documentHeight * 0.75}/${documentHeight}`);
            await new Promise(r => setTimeout(r, 1500));
            window.scrollTo({
                top: documentHeight,
                behavior: "smooth"
            ...
            const selectors = ["a:contains(\"查看更多\")", "button:contains(\"查看更多\")", "a:contains(\"展开\")", "button:contains(\"展开\")", "a:contains(\"更多\")", "button:contains(\"更多\")", "[data-spm-click*=\"展开\"]", "[class*=\"expand\"]", "[class*=\"more\"]", "[class*=\"ShowMore\"]", "[class*=\"ShowButton\"]"];
            ...
            for (const selector of selectors) {
                try {
                    if (selector.includes(":contains(")) {
                        const matchResult = selector.match(new RegExp("([a-z]+):contains\\(\"(.+)\"\\)", ""));
                        if (matchResult) {
                            const element = matchResult[1];
                            const searchTerm = matchResult[2];
                            const buttons = filterByText(element, searchTerm);
                            console.log(`找到 ${buttons.length} 个包含文本"${searchTerm}"的 ${element} 元素`);
                        ...
                        console.log(`找到 ${buttons.length} 个匹配选择器 "${selector}" 的元素`);
                        for (const btn of buttons) {
                            if (btn.offsetParent !== null) {
            ...
            const viewMoreButtons = filterByText("a", "查看更多评价").concat(filterByText("button", "查看更多评价"), filterByText("div", "查看更多评价"), filterByText("span", "全部评价"));
