function scrapeXiaohongshuSearchData(refresh = false) {
        ...
        try {
            ...
            function countNotes(noteCount) {
                ...
                for (const noteItem of noteItems) {
                    try {
                        const coverLink = noteItem.querySelector("a.cover") || noteItem.querySelector("a[href*=\"/explore/note/\"]");
                        if (!coverLink) {
                        ...
                        noteCount++;
                        const coverImage = noteItem.querySelector("img[data-xhs-img]") || noteItem.querySelector("img.cover");
                        const coverSrc = coverImage ? coverImage.src : "";
                        const noteTitle = noteItem.querySelector(".title span") || noteItem.querySelector(".content");
                        const noteTitle2 = noteTitle ? noteTitle.textContent.trim() : "";
                        const authorElement = noteItem.querySelector(".author") || noteItem.querySelector(".user-name");
                        const authorName = authorElement ? authorElement.textContent.trim() : "";
                        const authorLink = noteItem.querySelector(".author a") || noteItem.querySelector(".user-name a") || authorElement;
                        const authorLink2 = authorLink && authorLink.href ? authorLink.href : "";
                        const authorImage = noteItem.querySelector(".author img") || noteItem.querySelector(".avatar img");
                        const authorAvatarSrc = authorImage ? authorImage.src : "";
                        const likeCount = noteItem.querySelector(".like-wrapper .count") || noteItem.querySelector(".like-count");
                ...
                console.log(`本次新增 ${noteCount} 个笔记，当前总数: ${notesData.length}`);
                return noteCount;
            }
            async function loadMoreNotes() {
                ...
                try {
                    ...
                    if (noteCount === 0) {
                        noNewDataCount++;
                        console.log(`本次滚动没有新增笔记，无新数据计数: ${noNewDataCount}`);
                        if (noNewDataCount >= 3) {
                            console.log(`连续 ${noNewDataCount} 次没有新增笔记，可能已加载完全部内容`);
                            isProcessing = false;
                            resolve(notesData);
                            return;
                        }
                    ...
                    if (notesData.length > 0) {
                        console.log(`虽然出错，但已抓取到 ${notesData.length} 个笔记，返回现有数据`);
                        resolve(notesData);
                    } else {
                        reject(error);
                    }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "scrapeData") {
        ...
        scrapeXiaohongshuSearchData().then(results => {
