function scrapeXiaohongshuUserData(refresh = false) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (userIdElement) {
                userInfo.userId = userIdElement.textContent.replace("小红书号：", "").trim();
            ...
            if (ipLocation) {
                userInfo.ipLocation = ipLocation.textContent.replace("IP属地：", "").trim();
                console.log(`找到IP属地: ${userInfo.ipLocation}`);
            }
            const interactions = userInfoContainer.querySelectorAll(".user-interactions > div");
            ...
            function fetchNotes(noteCount) {
                const feedsContainer = document.querySelector("#userPostedFeeds") || document.querySelector(".feeds-container");
                if (!feedsContainer) {
                ...
                for (const noteItem of noteItems) {
                    try {
                        const noteLink = noteItem.querySelector("a.cover") || noteItem.querySelector("a[href*=\"/explore/note/\"]") || noteItem.querySelector("a[href*=\"/user/profile/\"]");
                        if (!noteLink) {
                        ...
                        noteCount++;
                        const noteImage = noteItem.querySelector("img[data-xhs-img]") || noteItem.querySelector("img.cover") || noteLink.querySelector("img");
                        const noteImageSrc = noteImage ? noteImage.src : "";
                        const noteTitle = noteItem.querySelector(".title span") || noteItem.querySelector(".content") || noteItem.querySelector(".desc");
                        const noteTitle2 = noteTitle ? noteTitle.textContent.trim() : "";
                        const username = userInfo.username;
                        const topTags = noteItem.querySelector(".top-tag-area .top-wrapper") || noteItem.querySelector(".top-wrapper");
                        const hasTopTags = !!topTags;
                        const likeCount = noteItem.querySelector(".like-wrapper .count") || noteItem.querySelector(".like-count") || noteItem.querySelector(".interact-info");
                ...
                console.log(`本次新增 ${noteCount} 个笔记，当前总数: ${userInfo.notes.length}`);
                return noteCount;
            }
            async function loadMore() {
                ...
                try {
                    ...
                    if (notesCount === 0) {
                        ...
                        if (noteCount2 >= 3) {
                            console.log(`连续 ${noteCount2} 次没有新增笔记，可能已加载完全部内容`);
                            isFetching = false;
                            resolve(userInfo);
                            return;
                        }
                    ...
                    console.log(`滚动加载更多内容，当前已有 ${userInfo.notes.length} 个笔记`);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "scrapeData") {
        ...
        scrapeXiaohongshuUserData().then(results => {
