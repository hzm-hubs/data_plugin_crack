function isXiaohongshuNoteDetail() {
    ...
    const noteId = window.location.href.match(new RegExp("\\/explore\\/([a-zA-Z0-9]+)", ""));
    const noteId2 = noteId && noteId[1];
    console.log("是否能从URL提取笔记ID:", noteId2 ? `是，ID: ${noteId[1]}` : "否");
    const noteContainer = document.querySelector(".note-container, #noteContainer, div[note-id]");
    ...
    if (noteContainer) {
        ...
        const interaction = noteContainer.querySelector(".interaction-container, .comments-el");
...
async function scrapeXiaohongshuNoteData(noteId, noteTitle, noteContent, createdAt, noteId2) {
    console.log("开始抓取小红书笔记详情数据...");
    console.log("当前URL:", window.location.href);
    noteId = "";
    const extractedNoteId = window.location.href.match(new RegExp("\\/explore\\/([a-zA-Z0-9]+)", ""));
    ...
    const authorInfoContainer = noteContainer.querySelector(".author-wrapper, .author");
    if (authorInfoContainer) {
        ...
        if (authorName) {
            ...
            if (authorLink && authorLink.href) {
                const userId = authorLink.href.match(new RegExp("\\/user\\/profile\\/([a-zA-Z0-9]+)", ""));
                if (userId && userId[1]) {
    ...
    createdAt = "";
    const creationDate = noteContainer.querySelector(".date, .note-content .date");
    if (creationDate) {
    ...
    const likeCount = noteContainer.querySelector(".interact-container .like-wrapper .count");
    if (likeCount) {
    ...
    const commentCount = noteContainer.querySelector(".interact-container .chat-wrapper .count");
    if (commentCount) {
    ...
    const collectCount = noteContainer.querySelector(".interact-container .collect-wrapper .count");
    if (collectCount) {
    ...
    noteId2 = "";
    let videoUrls = [];
    const videoElement = noteContainer.querySelector(".media-container video, video.xgplayer-video");
    if (videoElement) {
        noteId2 = "video";
        ...
        if (videoElement.src) {
            ...
            try {
                const playerElements = noteContainer.querySelectorAll(".xgplayer, .player-container");
        ...
        let videoCover = "";
        const videoPoster = noteContainer.querySelector(".xgplayer-poster, xg-poster");
        if (videoPoster) {
            const videoStyle = videoPoster.getAttribute("style");
            if (videoStyle) {
                const videoUrl1 = videoStyle.match(new RegExp("url\\(['\"]?(.*?)['\"]?\\)", ""));
        ...
        const imageElems = noteContainer.querySelectorAll(".media-container img, .note-detail-img img");
...
async function scrapeNoteComments(maxComments = 100) {
    ...
    try {
        ...
        const totalComments = totalCommentsElement ? totalCommentsElement.textContent.trim() : "";
        const totalMatch = totalComments.match(new RegExp("共\\s*(\\d+)\\s*条评论", ""));
        ...
        while (scrollAttempts < maxScrollAttempts) {
            const commentCount = commentsList.querySelectorAll(".parent-comment").length;
        ...
        for (const comment of loadedComments) {
            try {
                ...
                const userName = author ? author.querySelector(".name").textContent.trim() : "";
                const dateElement = comment.querySelector(".info .date span:first-child");
                const date = dateElement ? dateElement.textContent.trim() : "";
                const $location = comment.querySelector(".info .date .location");
                const $location2 = $location ? $location.textContent.trim() : "";
                const likeCount = comment.querySelector(".info .interactions .like .count");
                const likeCount2 = likeCount ? likeCount.textContent.trim() === "赞" ? "0" : likeCount.textContent.trim() : "0";
                ...
                const authorIdMatch = authorLink.match(new RegExp("\\/user\\/profile\\/([a-zA-Z0-9]+)", ""));
                const authorId = authorIdMatch ? authorIdMatch[1] : "";
                const tagElement = author.querySelector(".tag");
                const isAuthorTag = tagElement ? tagElement.textContent.includes("作者") : false;
                const replyCount = comment.querySelector(".info .interactions .reply .count");
                const replyCount2 = replyCount ? replyCount.textContent.trim() === "回复" ? "0" : replyCount.textContent.trim() : "0";
                comments.push({
                    id: commentId,
                    userName: userName,
                    content: note,
