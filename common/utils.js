...
function getParentChain(element, maxLevels = 5, level) {
    ...
    while (currentElement && level < maxLevels) {
        const classDetails = currentElement.className ? ` (class="${currentElement.className}")` : "";
        const idDetail = currentElement.id ? ` (id="${currentElement.id}")` : "";
        parentHierarchy.push(`${currentElement.tagName.toLowerCase()}${classDetails}${idDetail}`);
