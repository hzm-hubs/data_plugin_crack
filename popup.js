document.addEventListener("DOMContentLoaded", function (dataType, domain) {
  ...
  function fetchBaseUrl() {
    chrome.storage.local.get(["feishu_base_url"], function (result) {
      if (result.feishu_base_url) {
        document.getElementById("feishuBaseUrl").value = result.feishu_base_url;
      }
    });
  }
  function saveSettings() {
    const baseUrl = document.getElementById("feishuBaseUrl").value.trim();
    if (baseUrl) {
  ...
  function fetchData(tabId) {
    ...
    document.getElementById("status").innerText = "正在抓取数据...";
    let intervalId = null;
      ...
      chrome.tabs.query(
        ...
        function (tabs, fetchTime) {
          ...
          chrome.storage.local.get([storageKey], function (result) {
            ...
            chrome.storage.local.get(["feishu_base_url"], function (result) {
              let feishuUrl = result.feishu_base_url || "";
              if (!feishuUrl) {
              ...
              document.getElementById("status").innerText =
                "正在导出数据到飞书多维表格...";
              fetch(chrome.runtime.getURL("workflow_config.json"))
                ...
                .then((config) => {
                  ...
                  fetch("https://api.coze.cn/v1/workflow/run", {
                    ...
                    .then((data) => {
                      console.log("工作流API详细响应:", JSON.stringify(data));
                      if (data.code === 0 && data.data) {
                        try {
                          const response = JSON.parse(data.data);
                          console.log("解析后的响应数据:", response);
                          if (response.need_auth && response.auth_info) {
                            const isConfirmed = confirm(
                              "需要授权访问飞书多维表格。点击确定打开授权页面。"
                            );
                            if (isConfirmed) {
                              window.open(response.auth_info, "_blank");
                            }
                            document.getElementById("status").innerText =
                              "请完成授权后再次尝试导出";
                            return;
                      ...
                      if (data.code !== 0) {
                        console.error("工作流执行失败详情:", data);
                        if (
                          data.msg &&
                          data.msg.includes("function execution failed")
                        ) {
                          console.log(
                            "函数执行失败，可能是数据格式问题。数据类型:",
                            dataType
                          );
                          console.log(
                            "数据示例:",
                            JSON.stringify(exportData).substring(0, 500) + "..."
                          );
                        }
                      }
                      if (data.code === 0) {
  ...
  if (currentDomain === "taobao" || currentDomain === "tmall") {
    ...
    if (currentDomain === "douyin") {
      if (dataType === "search") {
        ...
        `;
              document.getElementById("status").innerText = "数据有误";
              return;
            }
