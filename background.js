...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    if (request.action === "updateData") {
        console.log("收到数据更新消息，域名:", request.domain, "数据类型:", request.type, "页面URL:", request.url);
        if (request.domain === "douyin") {
            if (request.type === "video") {
                console.log("抖音视频详情数据:", request.data.title);
            } else {
                if (request.type === "user") {
                    console.log("抖音用户数据:", request.data.nickname, "视频数量:", request.data.videos ? request.data.videos.length : 0);
                } else {
            ...
            if (request.domain === "xiaohongshu") {
                if (request.type === "user") {
                    console.log("小红书用户数据:", request.data.username, "笔记数量:", request.data.notes ? request.data.notes.length : 0);
                } else {
                    if (request.type === "search") {
