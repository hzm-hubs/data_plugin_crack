...
async function extractData(pageType) {
	...
	if (currentDomain == "one") {
		...
		pageType = "tableList";
	} else if (currentDomain == "sycm") {
		if (currentUrl.includes("/search_rank")) {
			console.log("检测到生意参谋排名页面，开始抓取排名数据");
			fetchedData = await scrapeSearchRank();
			pageType = "rank";
		} else if (currentUrl.includes("/search_analysis")) {
			console.log("检测到生意参谋分析页面，开始抓取分析数据");
			fetchedData = await scrapeSearchAnalysis();
			pageType = "analysis";
		}
